...
def generate_charts(file_path, output_dir=None):
    ...
    try:
        ...
        try:
            ...
            print(f"ERROR: No se pudo generar la matriz de similitud en forma de escalera: {str(e)}")
            plt.close()
        ...
        if len(sys.argv) > 1:
            ...
            try:
                ...
                if not file_path.endswith('.xlsx'):
                    print("ERROR: El archivo debe ser un archivo Excel (.xlsx)")
                    sys.exit(1)
                
                # Verificar que el directorio de salida sea válido
                if output_dir and not os.path.exists(output_dir):
                    try:
                        os.makedirs(output_dir, exist_ok=True)
                    except Exception as e:
                        print(f"ERROR: No se pudo crear el directorio de salida: {str(e)}")
                        sys.exit(1)
                ...
                # Errores de validación esperados (como columnas faltantes)
                print(f"ERROR: {str(e)}")
                sys.exit(1)
                ...
                print("ERROR: No se pudo analizar el archivo Excel. Verifique su formato")
                sys.exit(1)
            except Exception as e:
                # Errores inesperados
